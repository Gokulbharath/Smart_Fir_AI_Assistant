...
import { getAnalyticsStats, type AnalyticsStats } from '../api/analyticsService';
...
const Analytics: React.FC = () => {
  ...
  const statsData = [
    {
      title: 'Total FIRs',
      value: stats.firs.total.toLocaleString(),
      change: '+12%',
      trend: 'up' as const,
      icon: FileText,
      color: 'bg-blue-600'
    },
    {
      ...
      value: stats.firs.pending.toString(),
      change: '-8%',
      trend: 'down' as const,
      icon: Clock,
      color: 'bg-yellow-600'
    ...
    {
      title: 'Approved',
      value: stats.firs.approved.toString(),
      change: '+15%',
      trend: 'up' as const,
      icon: CheckCircle,
      color: 'bg-green-600'
    },
    {
      ...
      value: stats.evidence.total.toString(),
      change: '+23%',
      trend: 'up' as const,
      icon: AlertTriangle,
      color: 'bg-purple-600'
  ...
  // Calculate percentages for crime types
  const totalCrimeCount = stats.crimeTypes.reduce((sum, crime) => sum + crime.count, 0);
  const crimeTypesWithPercentage = stats.crimeTypes.map(crime => ({
    ...crime,
    percentage: totalCrimeCount > 0 ? (crime.count / totalCrimeCount) * 100 : 0
  }));
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 dark:border-slate-700/50 p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 mt-1 font-medium">
                Real-time crime statistics and performance insights
              </p>
          ...
          <button
            ...
            disabled={refreshing}
            className="p-2 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-lg hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-all duration-200 disabled:opacity-50"
      ...
      {stats.hotspots.length > 0 && (
        <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 dark:border-slate-700/50 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {stats.hotspots.slice(0, 6).map((hotspot, index) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 dark:border-slate-700/50 p-6 text-center">
          ...
          <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">{stats.metrics.avgProcessingTime}</h3>
        ...
        <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 dark:border-slate-700/50 p-6 text-center">
          ...
          <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">{stats.metrics.approvalRate}</h3>
        ...
        <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 dark:border-slate-700/50 p-6 text-center">
          ...
          <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">{stats.metrics.activeOfficers}</h3>
